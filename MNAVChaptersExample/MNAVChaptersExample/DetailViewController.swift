...
func chaptersFromAsset(_ asset: AVAsset) -> [MNAVChapter]? {
  return MNAVChapterReader.chapters(from: asset) as? [MNAVChapter]
}

func update(_ target: DetailViewController, url: URL) {
...
class DetailViewController: UITableViewController {
  ...
  func configureView() {
    ...
    task = sess.downloadTask(with: url, completionHandler: { [weak self] srcURL, res, er in
  ...
  override func tableView(
    ...
    let cell = tableView.dequeueReusableCell(
      withIdentifier: id, for: indexPath
    )
